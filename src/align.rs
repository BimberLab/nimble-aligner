...
pub fn score(sequences: impl Iterator<Item = Result<DnaString, Error>>, 
  ...
  sequences.fold(vec![0; config.reference_genome_size],
    |mut acc, read| {
      ...
      let seq_score = pseudoalign(read, &index, config.score_threshold, config.discard_multiple_matches);
      ...
      if let Some(itr) = &mut reverse_sequences {
        ...
        rev_seq_score = Some(pseudoalign(reverse_read, &index, config.score_threshold, config.discard_multiple_matches));
      }
...
fn pseudoalign(sequence: Result<DnaString, Error>, reference_index: &PseudoAligner,
  match_threshold: usize, discard_multiple_matches: bool) -> Option<(Vec<u32>, usize)> {
  if sequence.is_err() {
    return None;
  }

  match reference_index.map_read(&sequence.unwrap()) {
    Some((equiv_class, score)) => filter::align::filter_by_alignment_score(score, equiv_class, match_threshold, discard_multiple_matches),
    None => None
  }
}