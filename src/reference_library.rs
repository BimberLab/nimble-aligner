...
// Parses a nimble reference library. Returns a tuple of the library's aligner config information and the library data
// The nimble library format is defined here: https://github.com/BimberLab/nimble/wiki/Library-Format-Specification
pub fn get_reference_library(path: &Path) -> (align::AlignFilterConfig, ReferenceData) {
    // Parse the raw JSON to a serde_json value
    ...
    // Get the aligner configuration from the first JSON object in the file
    let config_obj = &v[0];
    ...
    let sequence_name_idx =
        get_column_index(&headers, "sequence_name").expect("Error -- could not find header sequence_name");

    // If there is no specified group_on column, group on the sequence name, which is the
    // highest-detail possible grouping level
    ...
    let sequence_idx =
        get_column_index(&headers, "sequence").expect("Error -- could not find header sequence");

    // Parse the data columns into a vector matrix of strings
    let columns = columns
        .as_array()
    ...
    // Pack data into the returned structs
    ...
    let reference_metadata = ReferenceData {
        group_on,
        headers,
        columns,
        sequence_name_idx,
...
// Convert a given serde_json value into a string array if possible, and panic otherwise
...
mod tests {
    ...
    // Test success case of to_string_vec
    #[test]
    fn to_string_vec() {
        let expected_results = vec![String::from("test1"), String::from("test2"), String::from("test3")];
        let results = super::to_string_vec(&json!(["test1", "test2", "test3"]), "test array");
        assert_eq!(results, expected_results);
    }

    // Test first panic in to_string_vec
    #[test]
    #[should_panic]
    fn to_string_vec_array_parse_error() {
        super::to_string_vec(&json!("not an array"), "test array");
    }

    // Test second panic in to_string_vec
    #[test]
    #[should_panic]
