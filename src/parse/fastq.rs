...
pub fn get_error_checked_fastq_readers(file_path: &str) -> (Box<dyn Iterator<Item = Result<DnaString, Error>>>, Box<dyn Iterator<Item = Result<DnaString, Error>>>) {
    (
...
fn get_error_checked_fastq_reader(file_path: &str) -> Box<dyn Iterator<Item = Result<DnaString, Error>>> {
    let (reader, _) = unwrap!(
    ...
    Box::new(fastq::Reader::new(reader)
        .records()
